...
export default function ProfilePage() {
  ...
  const userProfile = useMemo(() => {
    ...
    return {
      ...
      memberSince: u.createdAt
        ...
        : "—",
      location: u.isGuest ? (isEn ? "Guest player" : "Jugador invitado") : (isEn ? "Registered" : "Registrado"),
  ...
  const recentTickets = profile?.recentTickets ?? [];
  const strategyBreakdown = useMemo(() => {
    const empty = { count: 0, wins: 0, winRate: 0, totalPoints: 0, color: "#7c3aed", label: "Full Point", best: 0 };
    ...
    if (!s) {
      return {
        ...
        smart: { ...empty, color: "#f59e0b", label: "Smart Pick" },
      };
    }
    return {
      full: mapStat(s.full_point, "Full Point", "#7c3aed"),
      dual: mapStat(s.dual_point, "Dual Point", "#06b6d4"),
      smart: mapStat(s.smart_pick, "Smart Pick", "#f59e0b"),
    };
  }, [profile]);

  const circumference = 2 * Math.PI * 36;
  const winRateOffset = circumference - ((userProfile?.winRate ?? 0) / 100) * circumference;
  ...
  if (!isAuthenticated || !userProfile) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex flex-col items-center justify-center gap-4 text-center px-4">
        <p className="text-zinc-400">{isEn ? "Sign in to view your profile." : "Inicia sesion para ver tu perfil."}</p>
              ...
              >
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex justify-between text-zinc-400">
                    ...
                    <span className="font-bold" style={{ color: strat.color }}>{(strat.totalPoints ?? 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-zinc-400">
